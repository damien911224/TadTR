...
from .ops.deform_attn.modules import MSDeformAttn
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, pos_1d_embeds, pos_2d_embeds, query_embed=None, attn_mask=None):
        """
        Input:
            - srcs: List([bs, c, h, w])
            - masks: List([bs, h, w])
...
class DeformableTransformerCrossEncoderLayer(nn.Module):
    ...
    def forward(self, src, memory, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        # self attention
        ...
        # cross attention
        src2 = self.cross_attn(self.with_pos_embed(src, pos), reference_points,
                               memory, spatial_shapes, level_start_index, padding_mask)
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                if reference_points.shape[-1] == 4:
                    ...
                    new_reference_points[..., :2] = tmp + new_reference_points[..., :2]
