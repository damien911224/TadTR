...
class SetCriterion(nn.Module):
    ...
    def loss_QQ(self, outputs, targets, indices, num_segments):
        ...
        target_Q_weights = torch.softmax(torch.bmm(torch.log(C_weights), torch.log(C_weights).transpose(1, 2)), dim=-1)
        print(torch.max(target_Q_weights[0].detach().cpu(), dim=-1)[0][:10].numpy())
        print(torch.max(target_Q_weights[0, 0].detach().cpu(), dim=-1).numpy())
        ...
        loss_QQ = torch.sum(-tgt_QQ * torch.log(src_QQ + 1.0e-7), dim=-1)
