...
class TadTR(nn.Module):
    ...
    def forward(self, samples):
        ...
        for i in range(len(Q_weights) - 1):
            normalized_Q_weights = torch.sqrt(torch.bmm(normalized_Q_weights, Q_weights[i + 1]))
        ...
        for i in range(len(Q_weights) - 1):
            normalized_K_weights = torch.sqrt(torch.bmm(normalized_K_weights, K_weights[i + 1]))
