...
def apply_nms(dets_arr, nms_thr=0.4, use_soft_nms=True):
    ...
    for cls in unique_classes:
        ...
        if not use_soft_nms:
            ...
            b = torch.from_numpy(np.ascontiguousarray(this_cls_dets[..., :2]))
            s = torch.from_numpy(np.ascontiguousarray(this_cls_dets[..., 2]))
            l = torch.from_numpy(np.ascontiguousarray(this_cls_dets[..., 3].contiguous()))
    ...
    def accumulate(self, test_slice_overlap=0):
        ...
        for nms_mode in self.nms_mode:
            ...
            self.pred_by_cls[nms_mode] = [
                self.all_pred[nms_mode][self.all_pred[nms_mode].cls == cls].reset_index(drop=True).drop('cls', 1) for
                cls in range(self.num_classes)]
